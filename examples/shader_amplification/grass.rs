...
fn init_mesh_pipelines(
    ...
    // let task_shader = asset_server.load::<Shader>("shaders/mesh_shader/task.wesl");
    let mesh_shader = asset_server.load::<Shader>("shaders/mesh_shader_grass/mesh.wesl");
    let fragment_shader = asset_server.load::<Shader>("shaders/mesh_shader_grass/fragment.wesl");
    ...
    let cached_pipeline_id = pipeline_cache.queue_mesh_pipeline(MeshPipelineDescriptor {
        label: Some("grass_pipeline".into()),
...
fn draw_mesh_shader_cubes(
    ...
    for (camera, _, target, depth, view_uniform_offset, resolution_override) in &mut views {
...

        {
            ...
            // Since this MeshPipeline *does not* have a task shader, this call
            // dispatches the mesh shader workgroups
            pass.draw_mesh_tasks(50, 1, 1);
        }
    }
}
